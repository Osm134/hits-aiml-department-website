...
export default function Home() {
  ...
  const [eventForm, setEventForm] = useState({ title: "", description: "", date: "", image: null, file: null });
  ...
  const openEditEvent = (event) => {
    ...
    setEventForm({ ...event, image: null, file: null });
    setEventModalOpen(true);
  };

  const handleEventChange = (e) => {
    const { name, value, files } = e.target;
    setEventForm({ ...eventForm, [name]: files ? files[0] : value });
  };

  const saveEvent = async () => {
    const { title, description, date, image, file } = eventForm;
    ...
    try {
      if (editingEvent) {
        await API.put(`/events/${editingEvent.id}`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      } else {
  ...
  const goToEvent = (index) => {
    ...
    eventTimerRef.current = setInterval(() => setActiveEvent(prev => (prev + 1) % events.length), 4000);
  };
  ...
  const saveHighlight = async () => {
    ...
    try {
      await API.post("/academic-highlights", { title: highlightTitle, description: highlightDesc });
      const res = await API.get("/academic-highlights");
    ...
    <div className="bg-gray-50 font-sans text-gray-800 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 pb-8 grid md:grid-cols-3 gap-8 items-center">
        <div className="md:col-span-2 space-y-4">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#0B3C78] leading-tight">Welcome to the HITS AIML Department</h1>
          ...
          <button onClick={() => navigate("/academics")} className="mt-4 px-6 py-3 bg-[#0B3C78] text-white rounded-lg shadow hover:bg-blue-900 transition">Explore Academics</button>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <div className="w-full h-56 sm:h-64 md:h-72 bg-gray-100 flex items-center justify-center">
            <img src="/hod.jpg" alt="Dr. Sumitha Bhashini - HOD AIML" className="h-full w-auto object-contain" />
          </div>
          <div className="px-5 py-4 text-center">
            <h3 className="text-base sm:text-lg font-semibold text-[#0B3C78]">Dr. Sumitha Bhashini</h3>
            <p className="mt-1 text-sm text-gray-600 font-medium">Head of the Department</p>
            <p className="text-sm text-gray-500">Artificial Intelligence & Machine Learning</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid md:grid-cols-3 gap-6 mb-12">
        ...
        <div className="md:col-span-2 bg-white rounded-2xl shadow-lg hover:shadow-xl transition p-4 flex flex-col">
              ...
              updates.length === 0 ? <p className="text-gray-500 text-sm">No updates found.</p> :
                updates.map(u => (
                  <div key={u.id} className="border-b pb-2 last:border-b-0 relative">
                    ...
                    {u.file_url && (
                      <a href={`${BASE_URL}${u.file_url}`} download className="absolute top-0 right-0 bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded text-xs flex items-center gap-1">
          ...
          <div className="text-center mt-3">
            <button onClick={() => navigate("/daily-updates")} className="bg-[#0B3C78] hover:bg-blue-900 text-white px-4 py-1 rounded-lg text-sm transition">View All</button>
          </div>
        </div>
          ...
          <div className="mt-4 relative flex flex-col items-center w-full">
            <div className="w-full max-w-3xl bg-gray-50 rounded-2xl shadow-lg transition transform hover:scale-105 overflow-hidden flex flex-col animate-slide-in">
              <div className="w-full overflow-hidden rounded-t-2xl">
                <img src={events[activeEvent].image_url ? `${BASE_URL}${events[activeEvent].image_url}` : "/placeholder.jpg"} alt={events[activeEvent].title} className="w-full h-auto object-contain" />
              </div>
              <div className="p-4 flex flex-col gap-2">
                ...
                <div className="flex justify-between items-center mt-3">
                  {events[activeEvent].file_url && (
                    <a href={`${BASE_URL}${events[activeEvent].file_url}`} download className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm flex items-center gap-1">
                      <Download className="w-3 h-3" /> Download
                    </a>
                  )}
      ...
      {eventModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md relative">
            ...
            <input type="text" name="title" placeholder="Title" value={eventForm.title} onChange={handleEventChange} className="w-full mb-2 border rounded px-2 py-1" />
            <textarea name="description" placeholder="Description" value={eventForm.description} onChange={handleEventChange} className="w-full mb-2 border rounded px-2 py-1" />
            <input type="date" name="date" value={eventForm.date} onChange={handleEventChange} className="w-full mb-2 border rounded px-2 py-1" />

            <label className="block mb-4">
              Event Photo:
              <input type="file" name="image" accept="image/*" onChange={handleEventChange} className="mt-1" />
            </label>

            <div className="flex justify-end gap-2">
              <button onClick={() => setEventModalOpen(false)} className="px-4 py-2 bg-gray-400 rounded hover:bg-gray-500">Cancel</button>
              <button onClick={saveEvent} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 relative">
            ...
            <div className="flex space-x-6 overflow-x-auto py-2">
              {facultyList.map(f => (
                <div key={f.id} className="min-w-[200px] bg-white rounded-3xl shadow-xl overflow-hidden transform hover:scale-105 transition duration-300">
                  <img src={getFacultyImageURL(f.image_url)} alt={f.name} className="w-full h-48 object-cover" />
        ...
        <button onClick={() => navigate("/faculty")} className="fixed right-5 bottom-20 bg-[#0B3C78] text-white px-5 py-3 rounded-3xl shadow-lg hover:bg-blue-900 transition z-50">View All Faculty</button>
      </div>

      {/* QUICK LINKS */}
      <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-[#0B3C78] text-center">QUICK LINKS</h2>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-12">
        {quickLinks.map((link, idx) => (
          <Link key={idx} to={link.path} state={link.state} className={`flex flex-col items-center justify-center p-5 sm:p-6 rounded-2xl text-white font-semibold shadow-lg hover:scale-105 active:scale-95 transition-transform duration-200 bg-gradient-to-br ${link.color}`}>
            {link.icon}
      ...
      <footer className="bg-[#0B3C78] text-white py-8 mt-12">
        ...
        <div className="text-center mt-6 text-sm">&copy; {new Date().getFullYear()} AIML Department. All rights reserved.</div>
      </footer>
    </div>
  );
}
